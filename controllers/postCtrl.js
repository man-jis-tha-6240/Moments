...
const postCtrl = {
	createPost: async (req, res) => {
		try {
			...
			const newPost = new Post({
				content, imgs, user: req.user._id
			})
			await newPost.save()
			res.json({ msg: 'Post Created', newPost: { ...newPost._doc, user: req.user } })
		} catch (err) {
	...
	getPost: async (req, res) => {
		try {
			...
			const posts = await features.query.sort('-createdAt')
				.populate('user likes', 'avatar userName yourName followers')
				.populate({
					path: 'comments',
					populate: {
						path: 'user likes',
	...
	updatePost: async (req, res) => {
		try {
			...
			const post = await Post.findOneAndUpdate({ _id: req.params.id }, {
				content, imgs
			}).populate('user likes', 'avatar userName yourName').populate({
				path: 'comments',
				populate: {
					path: 'user likes',
					select: '-password'
	...
	getSinglePost: async (req, res) => {
		try {
			const post = await Post.findById(req.params.id).populate('user likes', 'avatar userName yourName followers').populate({
				path: 'comments',
				populate: {
					path: 'user likes',
					select: '-password'
	...
	deletePost: async (req, res) => {
		try {
			...
			res.json({msg:'Post deleted successfully',newPost:{
				...post,
				user:req.user
			}})
		} catch (err) {
